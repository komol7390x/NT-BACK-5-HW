...
@Injectable()
export class SallerQueryService {
  constructor(private readonly db: DBService) {}

  // ----------------------- CREATE SALLER TABLE -----------------------
  async sallerTable() {
    const exists = await this.checkTable('saller');
  ...
  async create(table_name: string, data: any) {
    if (table_name === 'saller') {
      await this.sallerTable();
    }

    const payload = data.createSallerDto ?? data;

    const keys = Object.keys(payload);
    const values = Object.values(payload);
    const query = keys.map((_, i) => `$${i + 1}`);
    // query
    const sql = `
  ...
  async findById(table_name: string, id: number = 1) {
    ...
    try {
      ...
      if (result[0] && result.length > 0) {
        ...
        return { message: false, data: 'not found Saller' };
      }
    } catch (error) {
      const data = error.detail;
      return { message: false, error: data };
  ...
  async update(table_name: string, data: any, id: number) {
    ...
    const payload = data.updateSallerDto ?? data;
